...
## 2026-08-31 审计结论 — S1 debug_server 标记已知接受

### S1: debug_server 监听 0.0.0.0:9090（安全项，**已知接受，不修**）

- **状态**: ⚠️ 已知接受（by design）
- **原因**: 该 HTTP 调试服务（`/debug/snapshot`、`/debug/route`、`/debug/rules`、`/debug/conns`、`/debug/logs` 等）
  主要用于**本地开发调试**（局域网内调试内核行为、验证路由决策、抓取请求流），
  用户明确判断其本地调试价值 > 局域网暴露风险。
- **缓解**: 调试接口无写磁盘/系统命令能力（仅内存态规则/连接查看与试验性路由演练），
  且仅在 VPN 内核启动时开启；若未来需要对外发布正式版本，需改为 127.0.0.1 或加
  鉴权开关（debug build 才启用）。
- **后续**: 审计中不再重复提出此项。
