...
- **首页连接状态位与实际隧道状态失配（严重度升级：安全语义问题，两端均可出现，非 UI 改造引入）**。
  - **现象**：
    - 此前记录形态（连上了却显示未连接）：`tun0` 已起、前台服务在运行（S24+ 上 `types=0x40000000`；API 28 无 `foregroundServiceType`，dumpsys 仅有 `isForeground=true`）、请求流与累计计数在走，首页却显示「未连接／连接」，速率显示 0 B/s；按 HOME 退出再进入即自愈为「已连接（加密隧道保护中）／断开」。
    - 本轮实测形态（界面显示已断开但隧道实际仍存活）：在 SO-02K（API 28，探针 release vc205）上实测，**点击断开后，界面显示「未连接／连接」，但 `ip addr show tun0` 显示 tun0 仍在、`dumpsys activity services` 显示前台服务仍有 1 个**。即隧道实际仍在运行而界面声称已断开。
  - **严重度定性**：这与此前记录的形态方向相反，但根因可能相同。对翻墙客户端而言，用户以为已断开、实际流量仍走代理，属于**安全语义问题**，不再只是显示问题。
  - **处置与分析**：需要单独一轮定位，不要在界面重构中顺手改。当前怀疑方向仍是首页状态只在 `onResume` 拉取、漏了前台期间到达的状态迁移，但**未查实，不下结论**。
