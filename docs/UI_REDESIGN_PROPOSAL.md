...
**C 阶段（Compose 探针）实测结果（2026-09-25）**：
- **测量设备**：Sony SO-02K（骁龙 835 / Android 9 / API 28 / 720×1280，无 wm 覆盖）
- **冷启动**：基线 release（main@17a1739，vc204）中位数 **500ms**（497–508）；Compose 探针 release（vc205）**520ms**（515–525），**+20ms / +4.0%**
- **APK 体积**：6,402,989 B → 6,953,710 B，**+550,721 B / +8.6%**
- **渲染**：基线 janky 0.96%（50th 8ms / 99th 15ms），探针 0.72%（50th 7ms / 99th 15ms），**未见劣化**
- **主进程 PSS**：36.0MB → 36.1MB，持平
- **功能冒烟**：R8 后功能冒烟通过（Compose 弹层正常渲染、6 项信息齐全、「管理内核」可拉起、「关闭」正常、连接/断开 tun0 起落正常、0 FATAL）
- **未覆盖**：S24+（API 36）未参与本轮 C 阶段测量；探针仅迁移了一个叶子界面（`SettingsActivity` 的版本弹层），未验证列表类界面的 Compose 渲染代价
- **已知缺陷**：Compose 内容未桥接应用主题，「管理内核」按钮显示为 Material3 默认紫，而全应用主色是 `#2481CC`。该桥接作为 C 阶段基建的一部分与探针一同落地。
...
本轮 §12.3 第 1 项把规则编辑从“立刻下发内核”改成“置脏标志、等用户点应用”，代码改完后有三处文案仍在描述旧行为，全部是实机点到才发现的：

1. `rules_empty`：「暂无自定义规则，点击下方「常用预设」或「添加规则」」——这两个按钮已搬到列表标题行，“下方”指向了不存在的位置。
2. `save_and_enable`：「保存并生效」（`dialog_edit_rule.xml:216` 引用）——保存已不再下发内核，文案与新行为直接矛盾，已修正为「保存」。
3. `rules_unapplied_hint`：初版写「规则有变动，需应用后生效」——暗示不应用就永不生效，与 `CoreService.kt:550` 每次服务启动注入 `RuleStore.toJson()` 的事实矛盾。已改为「改动需应用才影响当前连接」。

**规则**：**任何改变用户可见行为的改动，合并前必须检索所有描述该行为的字符串**（`res/values*/strings.xml` 全部语言目录），逐条判断是否仍然成立。代码编译通过、单测通过都发现不了这类错误——i18n 门禁只查键一致性与格式参数一致性，不查语义。

#### 视口矩阵：不靠攒设备，用 wm size / wm density 模拟

问题：实测结论只覆盖手上这两台（SO-02K 360×592dp、S24+ 384×832dp），无法回答“换一台别的尺寸分辨率会怎样”。

手法（本轮已验证可行）：在任意一台现有设备上覆盖显示配置，应用会按覆盖后的尺寸真实布局：

```bash
adb -s <serial> shell wm size 640x1280     # 配合 density 320 = 320x640dp
adb -s <serial> shell wm size 720x1040     # 配合 density 320 = 360x520dp
adb -s <serial> shell wm density <dpi>
adb -s <serial> shell wm size reset && adb -s <serial> shell wm density reset
```

**必须复位**：测完立刻 `wm size reset` + `wm density reset` 并回读确认（复位后 `wm size` 只剩 `Physical size:` 一行，没有 `Override size:`），否则设备会一直停在被覆盖的显示配置上。模拟视口截图会带黑边（letterbox/pillarbox），那是取景框，不是应用布局问题。

本轮用该手法测出的结果（这些是事实，照写）：

| 视口 | 来源 | 规则页结果 |
| --- | --- | --- |
| 360×592dp | SO-02K 真机 | 3 行完整，无截断 |
| 320×640dp | `wm size 640x1280` @320 模拟 | Geo 卡文案换 3 行、分应用卡换 2 行，把列表推下去，**第三行重新被切** |
| 360×520dp | `wm size 720x1040` @320 模拟 | 只剩 1 行完整 + 一条边 |

**结论与规则**：§12.3 第 1 项的修复只在 360dp 宽、592dp 高这一档成立。更窄会因为两张入口卡文案折行而失效，更矮则直接不够。规则：**任何“某某能完整显示 N 行/不截断”的验收标准，必须注明成立的视口档位**；跨档位的稳固性要么靠模拟矩阵验证，要么在文档里明说未验证。

#### 跨包性能对照必须在同等 dexopt 沉降条件下做

本轮实测：基线 release 包刚 `install` 完立即测，冷启动 7 次为 557/558/561/561/563/567/591ms（中位数 561ms）；同一个包在被反复启动、ART 后台 dexopt 完成后重测，为 497/497/499/500/502/507/508ms（中位数 500ms）。**同一个包沉降前后差 61ms**，而本轮要测的 Compose 差异只有 20ms——沉降噪声是待测信号的三倍。

若不控制该变量，会得出“探针比基线还快”（561ms vs 517ms）的假结论，且该结论方向讨喜、极难被质疑。

**规则**：跨包对照前，每个包都要先跑一轮预热（丢弃）再正式测量；测量前用 `adb shell dumpsys package dexopt | grep -A2 "\[com.mirage.android\]"` 确认状态为 `[status=speed-profile]`。

#### debug 构建的性能与体积数字不能用来评估发布代价

本轮实测同一份 Compose 改动：

| 指标 | debug 对照 | release 对照 |
| :--- | :--- | :--- |
| 冷启动 | +661ms（2242.5 → 2903.5ms） | **+20ms（500 → 520ms）** |
| APK | +7,163,900 B | **+550,721 B** |

差距约三十倍。原因两条：release 走 R8 剪枝；且 `ui-tooling` 是 `debugImplementation`，本就不进 release 包。

**规则**：凡是要据以做取舍的性能/体积结论，必须用 release 变体测。debug 数字只能用于“能不能跑通”的功能验证。

**附带可复用做法**：
- release 包若要装到已装 debug 包的设备上，用同一把调试密钥签名（经 `MIRAGE_KEYSTORE_PATH` 等环境变量注入，不改构建脚本），即可 `install -r` 而无需卸载。
- release 包不可降级安装（`-d` 只对 debuggable 包有效），换更高的 `versionCode` 重建即可。

### 11.5 已知遗留

- **首页连接状态位与实际隧道状态失配（严重度回调：定性更正为防御性加固，两端均可出现，非 UI 改造引入）**。
  - **实测形态与定性更正**：
    - **形态一（连着却显示未连接）——100% 必现，是真正要修的缺陷**：
      - 触发条件：隧道存活时 UI 进程被系统杀死（LMK 场景：`:core` 是前台服务 adj 100，UI 退后台后降为 adj 900 缓存进程），重新打开应用。退后台静置 10 秒再切回同样必现。
      - 实测表现：两台各 3 轮全部复现。`tun0` 已起、前台服务在运行（S24+ 上 `types=0x40000000`；API 28 无 `foregroundServiceType`，dumpsys 仅有 `isForeground=true`）、请求流与累计计数在走，首页却显示「未连接／连接」，速率显示 0 B/s。
      - 持续时间：界面显示「未连接」的持续时间 —— **SO-02K 约 500ms，S24+ 约 55ms**。
      - 用户行为危害：在此期间按钮绑定的是「连接」动作，用户看到「未连接」去点击，触发的是重复连接流程。
      - 根因分析（共两条）：
        1. `VpnRepository` 的 `telemetryJob` 缺少 `isBound()` 守卫（退后台 unbind 后 `isRunning()` 因为没有 binder 而非没有隧道返回 false，于是把状态覆写成 `Disconnected`）；
        2. `_vpnState` 初值硬编码为 `Disconnected`，而 `checkCurrentState()` 在未绑定时直接 return，新进程首帧就在断言一件它无从知晓的事。
    - **形态二（界面声称已断开而隧道仍存活）——代码路径确凿，但未证明用户可达**：
      - 机制确凿：`CoreController.call{}` 在 `service == null` 时静默返回 null，`stop()` 丢弃返回值；`stopVpn()` 随后无条件置 `Disconnected`。
      - 构造实验：人为制造未绑定窗口再调用断开逻辑，稳定抓到过完整现场：tun0 存活、前台服务存活、UI 宣告已断开。
      - 自然路径未触达：两台各 10 次正常「切回前台后立刻点击」，**0/10 撞进窗口**——点击均落在 `onServiceConnected` 之后 +275ms（SO-02K）/ +197ms（S24+）。
      - 极速点击实测：另在 LMK 冷启动场景下进行 6 次快速点击，点击一次都没被按钮接到（系统窗口接受输入的时刻晚于服务绑定完成）。合计 26 次尝试无一触达。
      - 原始定性更正：此前把它定为「安全语义问题」，依据是一次未复现的孤例观测（Compose 探针 release vc205 上「断开后 tun0 仍在」）。该观测未能复现，不足以支撑原定性。现按「代码路径存在、用户可达性未证实」记录，处置为防御性加固而非紧急修复。
  - **绑定窗口实测参考（同一构建 vc208，两台各 10 次，供后续参考）**：
    | 场景 | SO-02K (API 28) | S24+ (API 36) |
    | :--- | :--- | :--- |
    | 整包冷启动（含 `:core` fork） | 中位数 1000ms（991–1052） | 中位数 233ms（228–267） |
  ...
  - **修复状态（两项均已修复，在分支 `fix/vpn-state-sync`，尚未合入 main）**：
    ...
    - 证据强度标注：`CoreController.stop()` 未绑定那一支**没有实机端到端验证**——验证它原本依赖一个临时加的导出广播入口，该入口因安全漏洞被切除后证据随之失效。
    - 超时分支验证：两处 4s 超时的过期分支也只做了反向验证（确认健康路径不误触发），未构造出真实超时触发。
  - **方法论记录（孤例观测不足以支撑严重度定性）**：
    - 沿用本项目记录技术纠正的惯例（见 §12.1 与提交 `dbd4279`），把这次的教训记下来：
    - **孤例观测不足以支撑严重度定性。**
    - 本轮先有一次「断开后 tun0 仍在」的观测，据此把问题定为安全语义问题并排到最高优先级；
    - 26 次复现尝试后该形态未再出现，真正 100% 必现的是方向相反的形态一。
    - 定性应当等复现统计与因果链核验，而不是等一次令人警觉的观测。
...
#### 1. 规则页去掉底部三按钮
- **实现状态**：已落地，提交 `295e302`，分支 `ui/rules-chrome`（PR #13）。
- **改什么**：「应用」仍需用户显式确认（编辑仅置脏标志、滑出横幅由用户手动点击应用，不再即时下发内核；缓解事实为 `CoreService.kt:550` 每次服务启动注入 `RuleStore.toJson()`，未应用改动下次连接仍生效、只是不影响已建立的隧道）；「常用预设」下沉至列表标题行溢出菜单；「添加规则」改造为列表标题行图标按钮。同步核验并修正三处连带文案（`rules_empty`、`save_and_enable`、`rules_unapplied_hint`）。
- **改哪些文件**：`res/layout/fragment_rules.xml`、`RulesFragment.kt`、`RulesViewModel.kt`、`res/values/strings.xml` 等。
- **实测效果**：根治 360×592dp 视口下的第三行截断。常驻按钮条本身高 44dp；列表可用高度由改前约 187dp 增至改后约 215dp，两个数均由改前/改后截图按像素反算得出，差值 28dp 小于 44dp——未逐项核对布局树，不对这 16dp 的去向作解释。
- **验收标准（实测修正）**：**「360×592dp 下规则列表 3 行完整、无截断」**（SO-02K versionCode 193 实测验证通过）。
  - **原「≥4 行」未达成且原估算有误**：原案按规则行高 48–50dp 估算，实测约 70dp；4 行需 280dp，该页只给 215dp。去掉底部按钮条后实测增加约 28dp，根治了本阶段要解决的第三行截断，但 4 行还差约 65dp，只能从两张入口卡、列表标题行或悬浮胶囊底栏里取，三者都属 D 阶段，记为 D 阶段目标。
  - **已知视口边界**：该修复只在 360×592dp 视口成立。由 §11.4 模拟矩阵实测证实：在更窄视口（320×640dp）下，Geo 卡文案换 3 行、分应用卡换 2 行，把列表推下去，**第三行重新被切**；在更矮视口（360×520dp）下，只剩 1 行完整 + 一条边。跨视口稳固性留待 D 阶段统一重构。
- **风险**：编辑未应用时需依赖横幅明确提示「改动需应用才影响当前连接」，避免用户误以为已即时生效或永久失效。
