...
- **预期效果**：移除常驻底栏买回 44dp 垂直空间，规则列表可用高度从约 187dp 恢复至约 215dp，根治 360×592dp 视口下的第三行截断。
- **验收标准（实测修正）**：**「360×592dp 下规则列表 3 行完整、无截断」**（SO-02K versionCode 193 实测验证通过）。
  - **原「≥4 行」未达成且原估算有误**：原案按规则行高 48–50dp 估算，实测约 70dp；4 行需 280dp，该页只给 215dp。去掉底部按钮条买回 44dp，根治了本阶段要解决的第三行截断，但 4 行还差约 65dp，只能从两张入口卡、列表标题行或悬浮胶囊底栏里取，三者都属 D 阶段，记为 D 阶段目标。
