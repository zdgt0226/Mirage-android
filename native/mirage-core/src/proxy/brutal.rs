...
/// Android (bionic) 内核同 Linux 一样支持 SO_COOKIE (uapi/linux/socket.h 值 67),
/// 但 libc crate 未对 `android` target 暴露该常量 —— MIRAGE_MOBILE 补丁。
/// 取不到 cookie 时调用方会放弃监测, 降级安全。
#[cfg(target_os = "android")]
const SO_COOKIE: libc::c_int = 67;
