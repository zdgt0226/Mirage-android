...
impl std::io::Write for PanelWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        mirage_core::monitor::recent_logs_push(&s);
    ...
    ($fallback:expr, $body:expr) => {
        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| $body)) {
            ...
            Err(e) => {
                ...
                tracing::error!("[mirage-jni] JNI boundary panic caught: {}", msg);
                eprintln!("[mirage-jni] JNI boundary panic caught: {}", msg);
                $fallback
            }
        }
    };
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_start(
    ...
    jni_boundary!(-1, {
        ...
        *CURRENT_NODE_URI.lock().unwrap_or_else(|e| e.into_inner()) = Some(uri_str);
        ...
        mirage_core::attribution::set_package_resolver(Box::new(
            move |proto, src_ip, src_port, dst_ip, dst_port| {
                use jni::objects::JValue;
        ...
        mirage_core::command_server::start_command_server(
            stack.stop_notify.clone(),
            Some(rt.handle()),
        );

        *RUNTIME.lock().unwrap_or_else(|e| e.into_inner()) =
            Some(Arc::new(RunState { engine, stack, rt }));
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_isRunning(
    ...
    jni_boundary!(0, { STARTED.load(Ordering::SeqCst) as jboolean })
}
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_getStats(
    ...
    jni_boundary!(std::ptr::null_mut(), {
        ...
        let tcp =
            mirage_core::tun::tcp::TCP_ACTIVE.load(std::sync::atomic::Ordering::Relaxed) as f64;
        let udp = mirage_core::tun::udp::flow_count_global() as f64;
        let dns =
            mirage_core::tun::dns::DNS_QUERIES.load(std::sync::atomic::Ordering::Relaxed) as f64;
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_getPoolSize(
    ...
    jni_boundary!(-1, { CURRENT_POOL_SIZE.load(Ordering::Relaxed) as jint })
}
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_getBuiltinIpCount(
    ...
    jni_boundary!(0, { mirage_core::direct::builtin_ip_count() as jlong })
}
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_loadGeoFiles(
    ...
    jni_boundary!(std::ptr::null_mut(), {
        let site_p: String = env
            .get_string(&geosite_path)
            .map(|s| s.into())
            .unwrap_or_default();
        let ip_p: String = env
            .get_string(&geoip_path)
            .map(|s| s.into())
            .unwrap_or_default();
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_closeAllConnections(
    ...
    jni_boundary!(0, { mirage_core::monitor::close_all_connections() as jint })
}
...
pub extern "system" fn Java_com_mirage_android_core_MirageNative_getOutboundMode(
    ...
    jni_boundary!(0, { mirage_core::direct::get_outbound_mode() as jint })
}
